...
function createSymbolWithDescription(property) {
  const mySymbol = Symbol(property); // Створюємо символ symbol з описом property
  return mySymbol.description; //Повертаємо опис символа symbol
}
...
function createSymbolProperty(property) {
  ...
  const obj = Object(); // Створимо пустий об'єкт
...
function useSymbolToStringTag() {
  ...
  const key = Symbol();
  myObject[key] = "CustomObject"; // Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
  return myObject[key].toString(); // Повертаємо рядкове представлення myObject[Symbol].
}
...
let rangeObject = {
  ...
  [Symbol.iterator]() {
    ...
    this.curent = this.from; // this.current присвоюємо this.from
    return this; // Повертаємо this
  },

  next() {
    //створюжмо метод "next" який визначає поведінку при кожній ітерації
    //return this.curent <= this.to ? { done: false, value: this.curent++ } : { done: true }; // Використовуйте if
    if (this.curent < this.to) {
      // Якщо current менше to, повертаємо об'єкт з властивістю "value",що містить поточне значення ,
      return { done: false, value: this.curent++ }; // та не забуваємо збільшити індекс за допомогою інкремент, і "done" - false, означаючи, що ітерація ще не закінчена
    } else {
